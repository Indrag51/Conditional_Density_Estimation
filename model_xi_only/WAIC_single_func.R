den.eval_model.1<- function(XI_mat, y, X, BETA_mat,
                    location,test_knots){
...
waic.eval_model.1 = function(XI_mat, y, X, BETA_mat, test_knots){
  K = ncol(y)
  samplesize = nrow(X)
  Niter = dim(BETA_mat)[3]
  dummy.mat = data.frame()
  for(i in 1:K){
    A = as.data.frame(den.eval_model.1(XI_mat, y[,i], X, BETA_mat,
