...
for (j in 1:ncol(theta.true)) {
  for(i in 1:nrow(X2)){
    # y_vec[(j-1)*nrow(X2) + i]<-ifelse(runif(1)<=1,rbeta(1,16*W_0[(j-1)*nrow(X2) + i],32*j/ncol(theta.true)),
    ...
    # y_vec[(j-1)*nrow(X2) + i]<- ifelse(runif(1)<=1/(1+exp(W_1[(j-1)*nrow(X2) + i])),
    #                                   rbeta(1,16*W_0[(j-1)*nrow(X) + i],16),
    #                                   rbeta(1,8,16*sin(W_0[(j-1)*nrow(X) + i])))
    y_vec[(j-1)*nrow(X2) + i]<- ifelse(runif(1)<=0.5,
                                      rbeta(1,8*W_0[(j-1)*nrow(X) + i],16),
                                      rbeta(1,10*exp(W_1[(j-1)*nrow(X) + i]),4))
...
test_knots = seq(0,1, length.out = 101)
...
Gamma1 = kronecker(CovMat.2(test_knots, nu= 2.5, sigma = 3,theta = .37),
                   CovMat.2(test_knots, nu= 2.5, sigma = 0.5,theta = .5))
CHOL1 = t(chol(Gamma1))
...
for(i in 1:length(location.1))
{
  ...
  for(j in 1:nrow(X_new)){
    a=a.vec[j]
    a1=a1.vec[j]
    density.true<- dbeta(grid,8*a,16)*0.5 + dbeta(grid,10*exp(a1),4)*0.5
    # density.true<- (dbeta(grid,16*a,32*location.1[i]/ncol(theta.true)) * 4 +
