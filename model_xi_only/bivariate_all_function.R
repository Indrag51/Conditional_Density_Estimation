...
#library(mcmcplots)
library(reshape2)
sourceCpp("bivariate_function.cpp")
...
get_updates<- function(xi,beta,X,y,nmcmc = 2500,burnin = 2500,
  ...
  print(proc.time()-t0)
  return(list(XI_mat.out = XI_mat, BETA_mat.out = BETA_mat,
...
######### Calculate Quantiles ################
get_quantile = function(xi, beta, X_pred, test_knots, prob=0.5){
  knot_length = length(test_knots)
  intsize = 1/(knot_length-1)
  p=nrow(beta)
  K=ncol(beta)
