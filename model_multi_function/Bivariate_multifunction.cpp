...
//   http://gallery.rcpp.org/
  //
  // log f(y|X,s) ~ \sum_s g_s(y,x^T\beta(s))
  // [[Rcpp::export]]
...
// [[Rcpp::export]]
List logcond_likelihood_beta(arma::mat xi, arma::mat beta,
                             arma::mat X_new, arma::mat h_y, 
                             arma::vec test_knots){
  double knot_length = test_knots.n_rows;
  ...
  double temp=0;
  for(int i=0;i<K;i++){
    arma::mat xi_tilde =(xi.rows(i*knot_length,(i+1)*knot_length-1))*(H_Z.rows(i*n,(i+1)*n-1)).t();
  ...
  cons = cons + sum(log(colsums_cpp((exp(A2)-exp(A1))/((A2-A1)/intsize)))) + n*a_max;
  temp = temp + dot(vectorise((h_y.rows(i*n,(i+1)*n-1)),1),vectorise(xi_tilde,0));
  }
  if(exp(cons) <=0){
...
// [[Rcpp::export]]
List logcond_likelihood(arma::mat xi, arma::mat beta, arma::mat X_new,
