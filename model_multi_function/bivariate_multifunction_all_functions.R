...
mat_mean = function(x,l1,l2)
{
  return(matrix(x,nc=l1,nr=l2))
}
create_mat_mean = function(A){
  knot_length = ncol(A)
  K = nrow(A)/knot_length
  return(do.call(rbind,lapply(lapply(1:K, function(x) return(mean(A[((x-1)*knot_length+1):(x*knot_length),]))),
         function(x) mat_mean(x,knot_length,knot_length))))
}
...
ESS_xi = function(xi, xi_0, beta, logL, H_Z, 
  ...
  #xistar = (xistar - create_mat_mean(xistar))
  logLstar = logcond_likelihood(xistar,beta,X_new,h_y,test_knots,H_Z)$L
...
ESS_beta = function(xi, beta, beta_0, logL, 
                    h_y,X_new,test_knots){
  thetamin = 0
  thetamax = 2*pi
  u = runif(1)
  ...
  bo<- logcond_likelihood_beta(xi,betastar,X_new,h_y,test_knots)
  logLstar = bo$L
  H_Z_mat = bo$H_Z.val
...
get_updates<- function(xi,beta,X,y,nmcmc = 2500,burnin = 2500,
  ...
  knot_length = length(test_knots)
  XI_mat<- array(0, dim = c(knot_length*K,knot_length,nmcmc/thining))
  BETA_mat<- array(0,dim = c(p, K, nmcmc/thining))
  #TAU_mat<- matrix(0,nr=knot_length,ncol =  nmcmc/thining)
  ...
  for(i in 1:(nmcmc+burnin)){
    ...
    result_beta <- ESS_beta(xi, beta, beta_0, logL, 
                            h_y,X_new,test_knots)
    ...
    xi_0<- do.call(rbind,lapply(1:K,
                         FUN = function(x){A.temp = matrix(CHOL1%*%rnorm(knot_length^2),nc=knot_length,byrow = T); return(A.temp-mean(A.temp))}))
    #xi_0 = (xi_0 - create_mat_mean(xi_0))
    result_xi = ESS_xi(xi, xi_0, beta, logL, H_Z, 
                       h_y,X_new,test_knots)
    xi = result_xi$xi
    logL = result_xi$logL
    #####tau_update#######
    # tau_0 = rnorm(Nplus)
    # #tau_0 = tau_0-mean(tau_0)
...
PostFun_loc<- function(XI_mat, y_grid, X_new, BETA_mat,
                       location,test_knots){
  knot_length = length(test_knots)
  intsize = 1/(knot_length-1)
...
######### Calculate Quantiles ################
get_quantile = function(xi, tau, zeta, omega, beta, X_pred, test_knots, test_knots2, prob=0.5){
  knot_length = length(test_knots)
  intsize = 1/(knot_length-1)
  p=nrow(beta)
  K=ncol(beta)
