source("C:\\Users\\indra\\Dropbox\\Debdeep_Neuro\\Paper_final\\code\\Github\\model_multi_function\\bivariate_multifunction_all_functions.R")
source("C:\\Users\\indra\\Dropbox\\Debdeep_Neuro\\Paper_final\\code\\Github\\model_multi_function\\WAIC_multi.R")
library(ddalpha)
...
CHOL1 = t(chol(Gamma1))
xi = do.call(rbind,lapply(1:K1,
                          FUN = function(x){A.temp = matrix(CHOL1%*%rnorm(Nplus^2),nc=Nplus,byrow = T); return(A.temp-mean(A.temp))}))
...
for(i in 1:length(location.1))
{
  ...
  for(j in 1:nrow(X_new)){
    ...
    plot(grid, density.true,ylab = "",ylim= c(0, 6.5), type = "l",col="red",lwd=2,lty=1,
...
####################################
#save("run1", file = "bivariate_multifunction_function_implementation.Rdata")
