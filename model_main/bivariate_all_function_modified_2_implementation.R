source("/Users/indrajit_ghosh/Dropbox/Debdeep_Neuro/Paper_final/code/Github/model_main/bivariate_all_function_modified_2.R")
...
for (j in 1:K1) {
  for(i in 1:n1){
    # w_0 = (as.numeric(X2[i,]%*%theta.true[,j])+1)/2
    # a<-runif(1)
    # if(a<0.3)
    #   y1[i,j]<- rbeta(1,8*w_0,16*exp((j/ncol(theta.true)-0.5)/2))
    # if(a>=0.3 & a<0.7)
    #   y1[i,j]<- rbeta(1,4*j^{3/4},10*exp(w_0-0.5))
    # if(a>=0.7)
    #   y1[i,j]<-rbeta(1,3*j^{3/4},8*w_0*(2-j/K1))
...
#Gamma1 = kronecker(CovMat.2(test_knots, nu= 2.5, sigma = 4,theta = .70),
#                   CovMat.2(test_knots, nu= 2.5, sigma = 2,theta = .5))

Gamma1 = kronecker(CovMat.2(test_knots, nu= 2.5, sigma = 1,theta = .25),
...
for(i in 1:length(location.1))
{
  ...
  for(j in 1:nrow(X_new)){
    ...
    # density.true<- (dbeta(grid,8*a,16*exp((location.1[i]/ncol(theta.true)-0.5)/2)) * 0.3 +
    #   0.4*dbeta(grid,4*location.1[i]^{3/4},10*exp(a-0.5)) + 
...
####################################
#save("run1", file = "bivariate_all_function_modified_2_implementation.Rdata")
X_pred=X_test
n=nrow(X_pred)
Niter=nmcmc/thining
...
for(j in 1: length(u)){
  for(i in 1:Niter){
    ...
    Median_mat[,,i] = get_quantile(xi = xi,tau = tau,beta = beta,
                                  test_knots = test_knots,
                                  prob = u[j],X_pred = X_pred)
    if(i%%100==0) print(c(i,j))
  }
  Median_mat.loc[,,j] = apply(Median_mat,MARGIN = c(1,2), mean)
}
...
for (k in 1:n) {
  ...
  a1<- (a-0.5)*2
  #A = pbeta(x.grid,16*a,32*location.1[i]/ncol(theta.true))
...
waic.eval(XI_mat, y_test, X_test, BETA_mat, TAU_mat, test_knots)
